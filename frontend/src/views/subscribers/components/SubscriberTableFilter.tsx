...
const SubscriberTableFilter = () => {
  ...
  const onIsActiveFilterChange = (selected: SingleValue<Option>) => {
    dispatch(setFilterData({ method: selected?.value }))
  }
  ...
  return (
    <Select<Option>
      ...
      value={options.filter((option) => option.value === method)}
      onChange={onIsActiveFilterChange}
    />
  )
}

export default SubscriberTableFilter
